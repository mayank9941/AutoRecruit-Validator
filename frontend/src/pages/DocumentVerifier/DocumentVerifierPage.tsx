...
import { ArrowLeft, FileCheck2, RefreshCw, Check, X as XIcon } from 'lucide-react';
...
const CandidateVerification: React.FC<{ profileId: string; candidateId: string }> = ({ profileId, candidateId }) => {
  ...
  const handleDecision = (verificationId: string, dec: 'verified' | 'rejected') => {
    decision.mutate({ verificationId, decision: dec, notes: noteDrafts[verificationId] || undefined });
  };
        ...
        <div className="flex flex-col gap-3">
          {data.verifications.map((v) => (
            <div key={v.id} className="bg-card border border-border rounded-3xl p-5 shadow-sm flex flex-col gap-3">
              ...
              {v.hr_decision ? (
                <div className="text-[11px] font-semibold text-muted border-t border-border pt-3">
                  Marked <span className="font-bold text-foreground">{v.hr_decision}</span> by{' '}
                  {v.verified_by || 'HR'}
                ...
                <div className="flex items-center gap-2 border-t border-border pt-3">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-success/10 border border-success/20 text-success"
                    title="Mark verified"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-danger/10 border border-danger/20 text-danger"
                    title="Mark rejected"
                  >
